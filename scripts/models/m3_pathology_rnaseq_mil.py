...
class RNASeqEmbedding(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 256, embed_dim: int = 64, dropout: float = 0.5):
        super().__init__()
    ...
    def forward(self, rnaseq_features: torch.Tensor) -> torch.Tensor:
        if rnaseq_features.ndim == 1:
            rnaseq_features = rnaseq_features.unsqueeze(0)
        return self.net(rnaseq_features.float())


class PathologyRNASeqMIL(PathologySpatialMIL):
    def __init__(self, feature_extractor: nn.Module, config: M3ModelConfig):
        ...
        self.classifier = nn.Sequential(
            nn.LayerNorm(pathology_slide_dim + config.rnaseq_embed_dim),
            nn.Dropout(config.dropout),
            nn.Linear(pathology_slide_dim + config.rnaseq_embed_dim, config.n_outputs),
        )

    def forward(self, tile_images: torch.Tensor, coords: torch.Tensor, rnaseq_features: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        fused_slide = torch.cat([slide_feature, rnaseq_embedding], dim=-1)

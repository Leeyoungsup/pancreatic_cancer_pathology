...
최종 TCGA-PAAD 후보 코호트는 `../../results/pancreatic_cancer_pathology/data_verification/tcga_survival_cohort_candidate.csv`에 저장하였으며, 총 160명의 환자가 포함되었다.
...
최종 CPTAC-PDAC 후보 코호트는 `../../results/pancreatic_cancer_pathology/data_verification/cptac_survival_cohort_candidate.csv`에 저장하였으며, 총 140명의 케이스가 포함되었다.
...
Omics 데이터는 TCGA RNA-seq protein-coding gene과 CPTAC RNA-seq gene symbol의 교집합을 사용하였다. 최종 공통 유전자는 18,879개이며, 목록은 `../../results/pancreatic_cancer_pathology/data_verification/common_data/common_genes_tcga_rna_protein_coding__cptac_rna.csv`에 저장하였다.
...
M3/M4 모델에서는 전체 18,879개 gene을 그대로 사용하면 환자 수 대비 입력 차원이 과도하게 커지므로, 추가로 literature-guided survival-associated compact RNA-seq feature set을 구성하였다. 먼저 PDAC driver alteration, molecular subtype, epithelial/mesenchymal transition, stromal remodeling, immune/inflammatory signaling, proliferation, hypoxia/metabolism, DNA damage repair와 관련된 문헌 기반 curated gene seed를 구성하고 중복 gene을 제거하였다. 이후 train split에서만 TCGA-PAAD와 CPTAC-PDAC 각각의 gene별 univariate Cox score test를 수행하고, 각 데이터셋의 Cox z-score를 Stouffer 방식으로 결합하여 meta survival association score를 계산하였다. Validation/test case의 survival label은 gene selection에 사용하지 않았다. 최종 literature-guided ranking은 공통 RNA-seq에 존재하는 curated gene을 우선 배치하고, 부족한 feature 수는 train split Cox ranking으로 채웠다. 이 ranking을 기준으로 상위 1,000개, 1,500개, 2,000개 gene set을 저장하였다. 최종 모델 입력으로 저장하는 값은 기존 z-score matrix에서 선택 gene만 추출한 것이다. 기본 M3/M4 실험에서는 literature-guided 1,500개 gene을 사용하고, 1,000개와 2,000개 gene set은 RNA-seq 입력 차원에 따른 ablation으로 비교한다.

Curated seed gene은 다음 문헌 근거를 바탕으로 구성하였다. `KRAS`, `TP53`, `CDKN2A`, `SMAD4`, chromatin remodeling gene, DNA damage repair gene은 PDAC genomic landscape와 TCGA 통합 분석에서 반복적으로 보고된 주요 alteration을 반영하였다. Classical/progenitor, basal-like/squamous, EMT 관련 gene은 Collisson, Moffitt, Bailey 등의 PDAC molecular subtype 연구를 기반으로 포함하였다. Stromal remodeling 및 immune/inflammatory gene은 tumor/stroma subtype과 immunogenic subtype, activated stroma signature를 반영하였다. Hypoxia/metabolism, proliferation/cell-cycle gene은 aggressive tumor biology와 survival-associated transcriptomic phenotype을 포착하기 위한 보조 seed로 포함하였다.

주요 근거 문헌:

- Collisson EA et al. Subtypes of pancreatic ductal adenocarcinoma and their differing responses to therapy. *Nature Medicine*. 2011. DOI: [10.1038/nm.2344](https://doi.org/10.1038/nm.2344)
- Moffitt RA et al. Virtual microdissection identifies distinct tumor- and stroma-specific subtypes of pancreatic ductal adenocarcinoma. *Nature Genetics*. 2015. DOI: [10.1038/ng.3398](https://doi.org/10.1038/ng.3398)
- Bailey P et al. Genomic analyses identify molecular subtypes of pancreatic cancer. *Nature*. 2016. DOI: [10.1038/nature16965](https://doi.org/10.1038/nature16965)
- Waddell N et al. Whole genomes redefine the mutational landscape of pancreatic cancer. *Nature*. 2015. DOI: [10.1038/nature14169](https://doi.org/10.1038/nature14169)
- Cancer Genome Atlas Research Network. Integrated genomic characterization of pancreatic ductal adenocarcinoma. *Cancer Cell*. 2017. DOI: [10.1016/j.ccell.2017.07.007](https://doi.org/10.1016/j.ccell.2017.07.007)
- Puleo F et al. Stratification of pancreatic ductal adenocarcinomas based on tumor and microenvironment features. *Gastroenterology*. 2018. DOI: [10.1053/j.gastro.2018.08.033](https://doi.org/10.1053/j.gastro.2018.08.033)
...
M3/M4용 축소 RNA-seq feature는 다음 경로에 저장하였다.

- `../../data/pancreatic_cancer_pathology/dst/RNAseq_selected/top_literature_guided_survival_cox_1000/`
- `../../data/pancreatic_cancer_pathology/dst/RNAseq_selected/top_literature_guided_survival_cox_1500/`
- `../../data/pancreatic_cancer_pathology/dst/RNAseq_selected/top_literature_guided_survival_cox_2000/`

각 경로에는 `selected_genes_top_literature_guided_survival_cox_{n}.csv`, 데이터셋별 `matrix_rnaseq_zscore_top_literature_guided_survival_cox_{n}.csv`, 케이스별 `{case_id}_rnaseq_top_literature_guided_survival_cox_{n}.npy`, metadata JSON을 저장하였다. Gene ranking, curated gene table, feature selection 요약은 `../../results/pancreatic_cancer_pathology/data_preprocessing/rnaseq_feature_selection/`에 저장하였다. 순수 Cox ranking 산출물인 `top_survival_cox_{n}`도 보조 비교용으로 함께 저장하되, M3/M4의 기본 입력은 literature-guided set으로 정의한다.
...
M3는 병리 이미지 feature와 RNA-seq feature를 결합한 모델이다. RNA-seq 입력은 공통 gene 기반 z-score feature를 사용하되, 전체 18,879개 gene을 직접 사용하는 대신 literature-guided survival Cox ranking 기준 상위 1,500개 gene feature를 기본 입력으로 사용한다. 이 모델은 형태학적 phenotype과 molecular profile의 상호보완성을 평가하기 위한 핵심 비교 모델이다.
...
현재 구현한 M1-M4는 환자 단위 overall survival(OS) risk score를 예측하는 weakly supervised WSI survival model이다. 모델 입력은 case별 병리 tile image와 tile coordinate이며, M2는 최소 임상정보(`age_years`, `sex`), M3는 RNA-seq feature, M4는 RNA-seq feature와 최소 임상정보를 추가한다.
...
RNA-seq branch는 M3/M4에서 사용한다. RNA-seq 입력은 공통 gene 기반 z-score vector를 사용하되, 전체 18,879개 gene을 직접 사용하는 대신 literature-guided survival Cox ranking 기준 상위 1,500개 gene feature를 기본값으로 사용한다. Feature selection은 validation/test label을 사용하지 않도록 train split 내부에서만 수행한다. 최종 논문 실험에서는 1,000개, 1,500개, 2,000개 gene set을 비교하여 RNA-seq 입력 차원에 대한 sensitivity analysis를 수행한다.
...
### 4.6 M3 Pathology + RNA-seq Model

M3는 M1의 pathology branch에 RNA-seq branch를 결합한 multimodal survival model이다. Pathology branch는 M1과 동일하게 frozen UNI/UNI2-h tile encoder, tile fusion MLP, latent tile risk scoring, risk-weighted/top-k pooling을 통해 slide-level pathology embedding `H_i`를 생성한다.

RNA-seq 입력 `m_i`는 literature-guided survival Cox ranking 기준 상위 1,500개 gene의 z-score vector이다. RNA-seq branch는 입력 차원이 병리 branch에 비해 직접적이고 강한 signal을 가질 수 있으므로, 작은 embedding bottleneck과 높은 dropout을 적용한다. 현재 구현에서는 `1500 -> 256 -> 64` MLP를 사용한다.
...
M3의 목적은 병리 형태학적 representation과 molecular profile이 상보적인 예후 정보를 제공하는지 평가하는 것이다. M3 구현 파일은 `scripts/models/m3_pathology_rnaseq_mil.py`이며, 주요 구성요소는 다음과 같다.

- `RNASeqEmbedding`
- `LatentRiskTopKPooling`
- `PathologyRNASeqMIL`

### 4.7 M4 Pathology + RNA-seq + Basic Clinical Model

M4는 병리 이미지, RNA-seq, 최소 임상정보를 모두 결합하는 최종 multimodal survival model이다. Pathology branch는 M1과 동일하고, RNA-seq branch는 M3와 동일한 `1500 -> 256 -> 64` embedding을 사용한다. Clinical branch는 M2와 동일하게 `age_years_z`, `sex_male`, `sex_female`을 embedding한다.
...
M4는 세 modality가 모두 있을 때의 최종 성능을 평가한다. 단, RNA-seq feature가 강한 shortcut으로 작동할 수 있으므로 M1, M2, M3, M4뿐 아니라 RNA-only baseline과 RNA shuffle ablation을 함께 비교해야 한다. M4 구현 파일은 `scripts/models/m4_pathology_rnaseq_clinical_mil.py`이며, 주요 구성요소는 다음과 같다.
...
M1-M4는 동일한 survival label 구조와 학습 loop를 사용한다. 현재 실험에서는 TCGA-PAAD와 CPTAC-PDAC를 합친 combined cohort를 사용한다. 학습에는 24개월까지 생존 라벨을 확정할 수 있는 complete-24m case만 포함한다. 이 기준을 적용하면 TCGA-PAAD 112 cases와 CPTAC-PDAC 93 cases, 총 205 cases가 학습 후보가 된다. M4는 age/sex가 모두 존재하는 case만 사용하므로, clinical 결측 여부에 따라 최종 학습 case 수가 줄어들 수 있다.
...
We constructed a multi-modal pancreatic cancer cohort from TCGA-PAAD and CPTAC-PDAC by selecting cases with available H&E whole-slide images, RNA-seq profiles, and overall survival information. Whole-slide images were tiled at 1.0 microns per pixel into 1024 x 1024 patches after excluding low-tissue background regions using low-resolution tissue masks. Clinical variables were harmonized into a common schema across datasets and stored as case-level JSON files. RNA-seq profiles were transformed to log2-scale expression values, restricted to 18,879 common protein-coding genes, and standardized using gene-wise z-score normalization within each dataset. For multimodal modeling, compact RNA-seq feature sets containing the top 1,000, 1,500, and 2,000 genes were generated by prioritizing literature-curated PDAC-relevant genes and then filling the remaining features using train split univariate Cox score tests followed by cross-dataset Stouffer meta-analysis. The final processed dataset consisted of 160 TCGA-PAAD cases and 140 CPTAC-PDAC cases with matched image tiles, clinical variables, RNA-seq features, and overall survival labels.
